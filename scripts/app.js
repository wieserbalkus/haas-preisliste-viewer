...
function buildListPrintMarkup(mode){
  // Druck-Spaltenklassen:
  //  col-artnr   – Artikelnummer der Listenposition
  //  col-kurz    – Kurztext / Titel zur Position
  //  col-beschr  – Ausführliche Beschreibung
  //  col-eh      – Mengeneinheit der Position
  //  col-ehinfo  – Zusatzinfos zur Einheit
  //  col-preis   – Einzelpreis (rechtsbündig)
  //  col-menge   – Menge (rechtsbündig)
  //  col-gesamt  – Gesamtpreis (rechtsbündig, ggf. negativ)
  //  col-hinweis – Hinweistext oder interne Notizen
...
document.getElementById('printSummary').addEventListener('click', ()=>{ doPrintSummary(); });

document.addEventListener('keydown',(e)=>{
  if(e.defaultPrevented) return;
  const key = String(e.key||'').toLowerCase();
  const meta = e.ctrlKey || e.metaKey;
  if(!meta || key!=='p' || e.altKey) return;
